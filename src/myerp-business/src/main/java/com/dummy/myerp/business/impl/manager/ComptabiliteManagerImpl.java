...
public class ComptabiliteManagerImpl extends AbstractBusinessManager implements ComptabiliteManager {
    ...
    public synchronized void addReference(EcritureComptable pEcritureComptable) {
        ...
        // TODO à implémenter
        //On obtient un calendar
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(pEcritureComptable.getDate());
        //On stocke la valeur
        SequenceEcritureComptable sequenceEcritureComptable = getDaoProxy().getComptabiliteDao().getSequenceEcritureComptableByJournalAndAnnee(pEcritureComptable.getJournal().getCode(), calendar.get(Calendar.YEAR));


        if (sequenceEcritureComptable == null) {
            sequenceEcritureComptable = new SequenceEcritureComptable(calendar.get(Calendar.YEAR), 0);

        }
        sequenceEcritureComptable.setDerniereValeur(sequenceEcritureComptable.getDerniereValeur()+1);
        DecimalFormat df = new DecimalFormat("00000");
        pEcritureComptable.setReference(pEcritureComptable.getJournal().getCode() + "-" + calendar.get(Calendar.YEAR) + "/" + df.format(sequenceEcritureComptable.getDerniereValeur()));

        getDaoProxy().getComptabiliteDao().insertOrUptadeSequenceEcritureComptable(sequenceEcritureComptable);
